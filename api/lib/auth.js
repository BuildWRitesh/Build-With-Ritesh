...
async function bootstrapAdmin() { await ensureSchema(); const email=String(process.env.ADMIN_EMAIL||'').trim().toLowerCase(); const hash=String(process.env.ADMIN_PASSWORD_HASH||''); if(!email||!hash) return; const sql=sqlClient(); await sql.query(`INSERT INTO users (id,email,password_hash) VALUES ($1,$2,$3) ON CONFLICT (email) DO UPDATE SET password_hash=EXCLUDED.password_hash, updated_at=NOW()`,[crypto.randomUUID(),email,hash]); }
...
module.exports = { issueSession, clearSession, session, requireAdmin, bootstrapAdmin, verifyPassword };
